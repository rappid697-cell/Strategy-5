...
app = FastAPI(title="BTC 15m · CUSUM", lifespan=lifespan)
...
state = {
    ...
    # ── CUSUM Change-Point accumulators (this build's UNIQUE state) ──────────────
    # Two-sided CUSUM over the per-tick directional evidence Z = (fair_up - 0.5):
    #   S+ = max(0, S+ + Z - K);  S- = min(0, S- + Z + K)
    # We ENTER only when one crosses H, then reset the crossed side (re-arm). Reset
    # both at every new market/window boundary. Persisted in save/load_state.
    "cusum_s_plus": 0.0,
    "cusum_s_minus": 0.0,
    "cusum_last_change": None,   # {"side","time","fair_up","market_id"} of the last fired change-point
    "cusum_window_start": None   # window the accumulators belong to (reset when it rolls)
}
...
def save_state():
    try:
        data_to_save = {
            ...
            # CUSUM change-point accumulators (persist so a restart mid-window keeps arming)
            "cusum_s_plus": state["cusum_s_plus"],
            "cusum_s_minus": state["cusum_s_minus"],
            "cusum_last_change": state["cusum_last_change"],
            "cusum_window_start": state["cusum_window_start"]
        }
    ...
    try:
        if os.path.exists("state_data.json"):
            with open("state_data.json", "r") as f:
                ...
                state["cusum_s_plus"] = loaded.get("cusum_s_plus", 0.0)
                state["cusum_s_minus"] = loaded.get("cusum_s_minus", 0.0)
                state["cusum_last_change"] = loaded.get("cusum_last_change")
                state["cusum_window_start"] = loaded.get("cusum_window_start")
...
async def update_loop():
    ...
    while True:
        try:
            ...
            # `current_price` is already the Chainlink WS value (RPC WS → Polymarket WS
            # → RPC REST), so open and close come from the SAME feed, keyed to the
            # market's own clock — the near-the-money "mistakes" from local-clock skew
            # disappear. Binance is only a fallback for the open when the bot was NOT
            # running at the window start (so we never saw the real open).
            ...
            # open captured at eventStartTime. If we didn't capture this window's open
            # (e.g. the bot wasn't running at the boundary), strike_open stays None and
            # NO trade is opened this window — we wait for the next window where we can
            # mark the real open.
            chainlink_open = win["chainlink"]
            strike_open = chainlink_open          # None until/unless captured
            strike_source = "chainlink_ws"

            # Model's fair-prob strike = the SAME authoritative open. If not captured
            # yet (joined mid-window), fall back to the Binance 5m open at
            # eventStartTime for the DISPLAY only — no trade opens without a real strike.
            # No Binance/spot fallback for the strike: if this window's real
            # Chainlink open wasn't captured, target_open stays None (the fair prob
            # is neutral and no trade opens) rather than scoring against a wrong ref.
            ...
            # (what Polymarket displays), not the ask we transact at. A positive edge
            # = the book hasn't repriced the move yet.
            ...
            # ── CUSUM CHANGE-POINT (this build's UNIQUE entry logic) ─────────────
            # Reset both accumulators at each new market/window boundary (a fresh
            # 15m contract starts a fresh evidence tally — no carry-over across the
            # roll). Keyed on `start_ms`, which is now the market's eventStartTime
            # (event_start_ms) — so the reset rolls with the market's real start, not
            # the local aligned clock.
            if state.get("cusum_window_start") != start_ms:
                if state.get("cusum_window_start") is not None:
                    log_message(f"CUSUM reset at window roll (S+={state['cusum_s_plus']:.2f} "
                                f"S-={state['cusum_s_minus']:.2f} -> 0)")
                state["cusum_s_plus"] = 0.0
                state["cusum_s_minus"] = 0.0
                state["cusum_window_start"] = start_ms

            # Accumulate per-tick directional evidence Z = (fair_up - 0.5): positive
            # when the fast feed leans UP, negative when it leans DOWN. K is the slack
            # (drift allowance) that keeps the accumulators from wandering on noise.
            change_point = None
            if fair_up is not None:
                Z = fair_up - 0.5
                k = settings.CUSUM_K
                H = settings.CUSUM_H
                state["cusum_s_plus"] = max(0.0, state["cusum_s_plus"] + Z - k)
                state["cusum_s_minus"] = min(0.0, state["cusum_s_minus"] + Z + k)
                # Fire on a crossing, then reset the crossed accumulator so it re-arms.
                if state["cusum_s_plus"] >= H:
                    change_point = "UP"
                    state["cusum_s_plus"] = 0.0
                elif state["cusum_s_minus"] <= -H:
                    change_point = "DOWN"
                    state["cusum_s_minus"] = 0.0
                if change_point:
                    state["cusum_last_change"] = {
                        "side": change_point,
                        "time": datetime.now().isoformat(),
                        "fair_up": fair_up,
                        "market_id": (str(poly_snapshot["market"].get("id"))
                                      if poly_snapshot.get("ok") else None),
                    }
                    log_message(f"CUSUM change-point: {change_point} (fair {fair_up:.3f}, "
                                f"S+={state['cusum_s_plus']:.2f} S-={state['cusum_s_minus']:.2f})")

            # Only enter on a crossing, and only if that side has a non-negative edge.
            decision = engines.decide_cusum({
                "changePoint": change_point,
                "mcProbUp": fair_up,
                "priceUp": market_up,
                "priceDown": market_down,
                "sPlus": state["cusum_s_plus"],
                "sMinus": state["cusum_s_minus"],
                # The change-point IS the trigger; the edge check is just a "don't chase
                # a turn the book already priced" guard, so require only non-negative edge.
                "evThreshold": 0.0,
            })
            ...
            state["latest_data"] = {
                ...
                "indicators": {
                    "fair": fair_data,
                    "cusum": {
                        "s_plus": state["cusum_s_plus"],
                        "s_minus": state["cusum_s_minus"],
                        "k": settings.CUSUM_K,
                        "h": settings.CUSUM_H,
                        "change_point": change_point,        # crossing fired THIS tick (or None)
                        "last_change": state["cusum_last_change"],  # most recent fired change-point
                    }
                },
                "analysis": {
